...
int main(int argc, char **argv)
{
    ...
    for (size_t j = (centerRabb - widthRabb); j < (centerRabb + widthRabb); ++j)
    {
        ...
        z(0, j) = (1.0 - (j - centerRabb) * (j - centerRabb) / float(widthRabb * widthRabb)) * maxRabb;
    }
    for (size_t j = (centerPred - widthPred); j < (centerPred + widthPred); ++j)
    {
        ...
        z(1, j) = (1.0 - (j - centerPred) * (j - centerPred) / float(widthRabb * widthRabb)) * maxPred;
    }
    write_snapshots snapshots;
    auto snap = [&snapshots, &CpuOutputPath](int n) {
...
/* 
    cpu_timer timer_gpu;
    integrate_const(runge_kutta4<matrix>(), prey_predator_system_gpu(2.4), z, 0.0, tmax, dt);//, boost::ref(obs));
    ...
    // std::cout << " -Cpu Multi-Thread Computation: " << run_time_gpu << "s" << std::endl;
